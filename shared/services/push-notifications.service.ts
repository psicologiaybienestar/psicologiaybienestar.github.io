...
export class PushNotificationsService {
  ...
  async unregister(): Promise<void> {
    try {
      const token = this.tokenSubject.value || this.getSavedToken();
      if (token) {
  ...
  async createChannels(): Promise<void> {
    try {
      ...
      const channels = [
        ...
        { id: 'noticias', name: 'Noticias', description: 'Nuevos articulos publicados', importance: 4 as any, visibility: 1 as any },
        { id: 'actividades', name: 'Actividades', description: 'Actividades de bienestar nuevas', importance: 3 as any, visibility: 1 as any },
        { id: 'minijuegos', name: 'Minijuegos pausados', description: 'Canal temporalmente silenciado', importance: 1 as any, visibility: 0 as any },
        { id: 'emociones', name: 'Emociones', description: 'Nuevas categorias emocionales', importance: 3 as any, visibility: 1 as any },
      ...
      console.warn('[FCM] Error creando canales:', e?.message || e);
    }
  }
  ...
  async saveToken(token: string): Promise<boolean> {
    try {
      ...
      console.warn('[FCM] No se pudo guardar en localStorage:', e);
    }
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.error(`[FCM] Excepcion upsert (intento ${attempt}/3):`, e?.message || e);
        if (attempt < 3) {
          await new Promise(r => setTimeout(r, 1000 * attempt));
        }
      }
  ...
  async diagnose(): Promise<Record<string, any>> {
    ...
    try {
      ...
      console.log(`[FCM] Diagnostico: platform=${status['platform']}`);
    } catch {
      console.warn('[FCM] Diagnostico: Capacitor no disponible');
    }

    try {
      const { PushNotifications } = await import('@capacitor/push-notifications');
      status['pushPlugin'] = true;
      console.log('[FCM] Diagnostico: Push plugin disponible');

      try {
        const perm = await PushNotifications.requestPermissions();
        status['permission'] = perm.receive;
        console.log(`[FCM] Diagnostico: permiso=${perm.receive}`);
      } catch { /* ignore */ }
    } catch {
      console.warn('[FCM] Diagnostico: Push plugin NO disponible');
    }
    ...
    console.log('[FCM] Diagnostico completo:', JSON.stringify(status, null, 2));
    return status;
  }

  private handleNotificationTap(data: any): void {
    if (!data) return;
    if (data.table === 'mini_games' || data.type === 'mini_games' || data.channelId === 'minijuegos') return;
