document.addEventListener("DOMContentLoaded", () => {
  const testimoniosContenedor = document.querySelector(
  ...
  fetch(csvUrl)
    ...
    .then((csvText) => {
      ...
      const comentariosAMostrar = comentarios
        .filter((c) => c["Nombre Completo"] && c["Tu Testimonio"])
        .slice(0, 12);
      ...
      if (comentariosAMostrar.length > 0) {
        comentariosAMostrar.forEach((comentario) => {
          ...
          if (testimonioCompleto.length > limiteCaracteres) {
            const textoTruncado =
              testimonioCompleto.substring(0, limiteCaracteres) + "...";
        ...
        testimoniosContenedor.addEventListener("click", (event) => {
          ...
          if (
            target.classList.contains("ver-mas") ||
            target.classList.contains("ver-menos")
          ) {
            const testimonioPara = target.closest(".testimonio");
            if (testimonioPara) {
              const textoTruncadoSpan =
                testimonioPara.querySelector(".texto-truncado");
              const textoCompletoSpan =
                testimonioPara.querySelector(".texto-completo");
        ...
        testimoniosContenedor.innerHTML =
          '<div class="swiper-slide"><p class="text-center text-gray-600">No hay comentarios para mostrar en este momento.</p></div>';
